#Function to create data frame of significant edges from correlation matrix
#The resulting data frame may be used in network visualization with the igraph package
#See ?p.adjust for more p-value adjustment methods

#One issue is that the False Discovery Rate FDR method of Benjamini-Hochberg (BH) procedure
#which is the default of this function is meant for independent variables. Several other
#resampling-based procedures are provided by:
#	Reiner, Yekutielli, & Benjamini. 2003. Identifying differentially
#	expressed genes using false discovery rate controlling procedures.
#	Bioinformatics 19, 368-375.
#Another method is the boostrap false discovery method (BS_FD or BSFD) which can handle dependency
#between variables. This method takes as inputs:
#	The threshold to keep associations for analysis (which becomes) the first selection cutoff
#	The false discovery rate, fdr, which is the number of acceptable type I errors (false positives) 
#		acceptable to the researcher
#	The risk (probability) that the researcher is willing to accept that the number of false
#		discoveries exceeds the specified fdr
#cor.method = the correlation method used in the original correlation matrix x. This must be duplicated for bootstrapping
#Berry and Widder 2014 found that Spearman and Pearson correlations in interaction networks had the most specificity.
#	Therefore, it is recommended to use these over other dissimilarity measures.
#Another issue is that sparsely occurring OTUs may be correlate with each other too well and produce no standard deviation from
#	their correlation estimate. One way to address this is to use mothur's sparcc() command to produce a sparse composition
#	correlations. Any correlations produced here yield an NA

edge_data_from_matrix<-function(community.matrix,multiple.adjust=c('BY','BSFD'),output.filename='edge_data',
